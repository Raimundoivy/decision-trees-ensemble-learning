...
def feature_engineering(df):
    ...
    df_with_ratio_features = (
        df.copy()
    )  # Start with a copy to avoid modifying the original df passed in
    df_with_ratio_features["debt_to_income_ratio"] = (
        df_with_ratio_features["debt"] / (df_with_ratio_features["income"] + 1e-6)
    )
    df_with_ratio_features["assets_to_debt_ratio"] = df_with_ratio_features[
        "assets"
    ] / (df_with_ratio_features["debt"] + 1e-6)
    df_with_ratio_features["loan_amount_to_income_ratio"] = df_with_ratio_features[
        "amount"
    ] / (df_with_ratio_features["income"] + 1e-6)
    df_with_ratio_features["seniority_to_age_ratio"] = df_with_ratio_features[
        "seniority"
    ] / (df_with_ratio_features["age"] + 1e-6)
    df_with_ratio_features["expenses_to_income_ratio"] = df_with_ratio_features[
        "expenses"
    ] / (df_with_ratio_features["income"] + 1e-6)
    ...
    poly_features_array = poly.fit_transform(
        df_with_ratio_features[all_numerical_features]
    )
    ...
    columns_to_drop_from_df_poly = [
        col for col in all_numerical_features if col in df_poly.columns
    ]
    ...
    columns_to_keep = [
        col
        for col in df_with_ratio_features.columns
        if col not in original_numerical_features
    ]
